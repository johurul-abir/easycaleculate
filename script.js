    ...
    try {
        switch (currentEquation) {
            ...
            case 10: // P = mv
                ...
                break;
            case 11: // K = p²/m
                result = calculateKineticEnergyFromMomentum(
                    values,
                    selectedVariable
                );
                break;
        ...
        document.getElementById(`input-${selectedVariable}`).value =
            result.value;
...
function calculateEquation2(values, target) {
    ...
    switch (target) {
        ...
        case "u":
            ...
            return {
                value: result_u.toFixed(3),
                steps: `u = (${s} - ½ × ${a} × ${t}²) ÷ ${t} = ${result_u.toFixed(
                    3
                )}`,
            };
        case "a":
            ...
            return {
                value: result_a.toFixed(3),
                steps: `a = 2 × (${s} - ${u} × ${t}) ÷ ${t}² = ${result_a.toFixed(
                    3
                )}`,
            };
        case "t":
            ...
            if (result_t < 0) {
                ...
                if (result_t2 >= 0) {
                    return {
                        ...
                        steps: `Using quadratic formula: t = ${result_t2.toFixed(
                            3
                        )}`,
                    };
                }
...
function calculateEquation6(values, target) {
    ...
    switch (target) {
        ...
        case "u":
            ...
            return {
                value: result_u.toFixed(3),
                steps: `u = (${h} - ½ × ${g} × ${t}²) ÷ ${t} = ${result_u.toFixed(
                    3
                )}`,
            };
        case "g":
            ...
            return {
                value: result_g.toFixed(3),
                steps: `g = 2 × (${h} - ${u} × ${t}) ÷ ${t}² = ${result_g.toFixed(
                    3
                )}`,
            };
        case "t":
...
function calculateForceEquation(values, target) {
    ...
    switch (target) {
        ...
        default:
            throw new Error(
                "Invalid target variable. Choose 'F', 'm', or 'a'."
            );
    }
}
...
function calculateWeightEquation(values, target) {
    ...
    switch (target) {
        ...
        default:
            throw new Error(
                "Invalid target variable. Choose 'w', 'm', or 'g'."
            );
    }
}
...
function calculateMomentumEquation(values, target) {
    ...
    switch (target) {
        ...
        default:
            throw new Error(
                "Invalid target variable. Choose 'p', 'm', or 'v'."
            );
    }
}
...
function calculateKineticEnergyFromMomentum(values, target) {
    ...
    switch (target) {
        ...
        case "p":
            ...
            return {
                value: result_p.toFixed(3),
                steps: `p = √(${K} × ${m}) = √${K * m} = ${result_p.toFixed(
                    3
                )}`,
            };
        ...
        case "m":
            ...
            return {
                ...
                steps: `m = (${p}²) ÷ ${K} = ${
                    p * p
                } ÷ ${K} = ${result_m.toFixed(3)}`,
            };

        default:
            throw new Error(
                "Invalid target variable. Choose 'K', 'p', or 'm'."
            );
    }
}
...
function calculateKineticEnergy(values, target) {
    ...
    switch (target) {
        ...
        default:
            throw new Error(
                "Invalid target variable. Choose 'K', 'm', or 'v'."
            );
    }
}
...
function calculatePotentialEnergy(values, target) {
    ...
    switch (target) {
        ...
        case "m":
            ...
            if (g * h === 0)
                throw new Error("Gravity × height cannot be zero.");
        ...
        default:
            throw new Error(
                "Invalid target variable. Choose 'U', 'm', 'g', or 'h'."
            );
    }
}
    ...
    switch (target) {
        ...
        case "theta":
            ...
            if (cosTheta < -1 || cosTheta > 1)
                throw new Error(
                    "Invalid value: cos(θ) must be between -1 and 1."
                );
            ...
            return {
                ...
                steps: `cos(θ) = ${W} ÷ (${F} × ${s}) = ${cosTheta.toFixed(
                    3
                )} ⇒ θ = cos⁻¹(${cosTheta.toFixed(3)}) = ${result_theta.toFixed(
                    3
                )}°`,
            };
...
function calculatePower(values, target) {
    ...
    switch (target) {
        ...
        default:
            throw new Error(
                "Invalid target variable. Choose 'P', 'W', or 't'."
            );
    }
}
...
function calculateOhmsLaw(values, target) {
    ...
    switch (target) {
        ...
        default:
            throw new Error(
                "Invalid target variable. Choose 'V', 'I', or 'R'."
            );
    }
}
